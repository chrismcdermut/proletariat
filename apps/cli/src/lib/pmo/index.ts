...
export async function createPMO(options: CreatePMOOptions): Promise<void> {
  ...
  // PRLT-1299: Local workflow status creation removed — statuses are now
  // managed by the provider (Linear, Jira, etc.). Custom template column
  // names are stored in settings for reference but not as workflow statuses.
