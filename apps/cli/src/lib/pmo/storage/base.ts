...
- These raw commands skip ticket lifecycle updates and break board sync.`,
      ...
      validFrom: ['ready', 'backlog'] as string[],
      executor: null,
      environment: 'host',
      permissionMode: 'full',
      modifiesCode: true,
...
**STOP:** After completing the above, your task is done. Do not take any further actions.`,
      ...
      validFrom: ['needs_review'] as string[],
      executor: null,
  ...
  for (const action of builtinActions) {
    ...
    const validFromJson = action.validFrom?.length ? JSON.stringify(action.validFrom) : null
    drizzle
      .insert(pmoActions)
      .values({
        id: action.id,
