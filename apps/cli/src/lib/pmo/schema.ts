 ...
 *
 * Dead tables (local ticket store, local workflows, vestigial tables) were
 * removed in PRLT-1299. The provider (Linear, Jira, etc.) is now the source
 * of truth for tickets and workflows.
 */
...
export const PMO_TABLES = {
  ...
  // Workflow engine (prlt owns this)
  ...
  labels: 'pmo_labels',
  // Ticket registry (provider-keyed)
...
export const PMO_SCHEMA_SQL = [
  PMO_TABLE_SCHEMAS.phases,
  PMO_TABLE_SCHEMAS.phase_templates,
  PMO_TABLE_SCHEMAS.projects,
  PMO_TABLE_SCHEMAS.categories,
  ...
  PMO_TABLE_SCHEMAS.ticket_templates,
  PMO_TABLE_SCHEMAS.label_groups,
