...
`,
  ...
  async moveTicket(id: string, column: string, position?: number): Promise<Ticket> {
    ...
    // Get ticket's project_id from the database (tickets are globally unique by ID)
    const ticketRow = this.db.prepare(`SELECT project_id FROM ${T.tickets} WHERE id = ?`).get(id) as { project_id: string }
    const projectId = ticketRow.project_id

    // Verify target column exists in ticket's project
  ...
  async deleteTicket(id: string): Promise<void> {
    // Get ticket to find its project_id (tickets are globally unique by ID)
    const existing = await this.getTicketById(id)
    if (!existing) {
      throw new PMOError('NOT_FOUND', `Ticket not found: ${id}`, id)
    }

    // Get ticket's project_id from the database row
    const ticketRow = this.db.prepare(`SELECT project_id FROM ${T.tickets} WHERE id = ?`).get(id) as { project_id: string }
    const projectId = ticketRow.project_id
  ...
  async listTickets(filter?: TicketFilter): Promise<Ticket[]> {
    // Project filter: use filter.projectId if provided, else currentProjectId
    // Special case: '__ALL__' means no project filtering (query all projects)
    const projectId = filter?.projectId ?? this.currentProjectId
    const filterByProject = projectId && projectId !== '__ALL__'
  ...
  private async getTicketById(id: string): Promise<Ticket | null> {
    // Ticket IDs are globally unique - no need to filter by project
    const row = this.db.prepare(`
      SELECT t.*, bt.column_id, bt.position, c.name as column_name
      FROM ${T.tickets} t
      LEFT JOIN ${T.board_tickets} bt ON t.id = bt.ticket_id AND t.project_id = bt.project_id
