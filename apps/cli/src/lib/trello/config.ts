...
export function loadTrelloConfig(db: Database.Database): TrelloConfig | null {
  ...
  return {
    ...
    boardId: getSetting(db, TRELLO_CONFIG_KEYS.boardId)
      ...
      || undefined,
    boardName: getSetting(db, TRELLO_CONFIG_KEYS.boardName)
      || undefined,
  }
}
...
/**
 * Save the Trello board ID and optionally board name.
 */
export function saveTrelloBoardId(db: Database.Database, boardId: string, boardName?: string): void {
