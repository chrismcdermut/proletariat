 ...
 * Uses API key + token auth (Trello's standard authentication model).
 */
 ...
 * - Environment variables PRLT_TRELLO_API_KEY/TRELLO_API_KEY and PRLT_TRELLO_TOKEN/TRELLO_TOKEN are set
 */
export function isTrelloConfigured(db: Database.Database): boolean {
  const hasDbConfig =
    getSetting(db, TRELLO_CONFIG_KEYS.apiKey) !== null
  ...
  const hasEnvToken = !!(process.env.PRLT_TRELLO_TOKEN || process.env.TRELLO_TOKEN)
...
/**
 * Save the default Trello board ID.
 */
 ...
 * Also checks PRLT_TRELLO_TOKEN and TRELLO_TOKEN environment variables.
 */
export function getTrelloApiToken(db: Database.Database): string | null {
  const envToken = process.env.PRLT_TRELLO_TOKEN || process.env.TRELLO_TOKEN
