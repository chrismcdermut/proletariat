/**
 * MCP Tmux Tool Overrides
 *
 * These tools interact directly with tmux (not through CLI commands),
 * so they need custom MCP implementations rather than auto-generation.
 */
...
export const tmuxOverrideNames = [
  'tmux_send_keys',
  'tmux_capture_pane',
  'tmux_start_session',
  'tmux_list_sessions',
...
export function registerTmuxOverrides(server: McpServer, _ctx: McpToolContext): void {
  // ── tmux_send_keys ───────────────────────────────────────────────────
  strictTool(server,
    'tmux_send_keys',
