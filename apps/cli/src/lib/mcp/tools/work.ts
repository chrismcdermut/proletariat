...
export function registerWorkTools(server: McpServer, ctx: McpToolContext): void {
  ...
  strictTool(server,
    'work_review',
    'Evaluate the output/PR for a ticket. Spawns an agent that decides whether to comment, fix, or both from context.',
    {
      ticket_id: z.string().describe('Ticket ID to review'),
    },
    async (params) => {
      try {
        const cmd = `prlt work review ${params.ticket_id}`
