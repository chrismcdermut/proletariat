...
export default class TemplateWorkflowDelete extends Command {
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(TemplateWorkflowDelete);

    const cmdArgs: string[] = [args.id];
    if (flags.force) cmdArgs.push('--force');
    if (flags.json) cmdArgs.push('--json');

    await this.config.runCommand('status:template:delete', cmdArgs);
  }
}
