...
export default class SelfUpdate extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!jsonMode) {
      this.log('')
      this.log(colors.primary('Proletariat CLI Self-Update'))
      this.log('')
    ...
    if (flags.check) {
      if (jsonMode) {
        ...
        this.log(`  Run ${colors.primary('prlt self-update')} to install`)
      } else {
  ...
  private printTroubleshootingHelp(pm: PackageManager): void {
    ...
    if (pm === 'npm') {
      ...
      this.log(`  See: ${colors.textMuted('prlt self-update --help')} or`)
