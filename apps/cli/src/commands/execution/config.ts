  ...
  private async handleSettingPrompt(
    ...
    switch (setting) {
      case 'defaultEnvironment': {
        const envChoices = [
          { name: 'host - Run directly on host machine (no sandbox)', value: 'host', command: 'prlt execution config --set "defaultEnvironment host" --json' },
          { name: 'sandbox - srt restrictions on host (lightweight isolation)', value: 'sandbox', command: 'prlt execution config --set "defaultEnvironment sandbox" --json' },
