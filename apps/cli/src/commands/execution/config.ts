  ...
  private async handleSettingPrompt(
    ...
    switch (setting) {
      case 'defaultEnvironment': {
        const envChoices = [
          { name: 'host - Run directly on host machine (no isolation)', value: 'host', command: 'prlt execution config --set "defaultEnvironment host" --json' },
          { name: 'sandbox - Run with srt restrictions (filesystem + network isolation)', value: 'sandbox', command: 'prlt execution config --set "defaultEnvironment sandbox" --json' },
          { name: 'devcontainer - Run in a devcontainer (full isolation)', value: 'devcontainer', command: 'prlt execution config --set "defaultEnvironment devcontainer" --json' },
