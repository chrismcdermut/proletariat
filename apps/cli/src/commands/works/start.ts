...
export default class WorksStart extends PMOCommand {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> TKT-001 TKT-002 TKT-003',
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      // If ticket IDs provided as args, use those directly
      if (ticketIds.length > 0) {
        selectedTicketIds = ticketIds
        this.log(styles.muted(`\nStarting work on ${selectedTicketIds.length} tickets: ${selectedTicketIds.join(', ')}\n`))
      } else {
        ...
        // Get tickets from the selected column
        ...
        // Prompt user to select which tickets to start
        const { selectedTickets } = await inquirer.prompt([
          {
            type: 'checkbox',
            name: 'selectedTickets',
            message: `Select tickets to start from "${columnName}" (SPACE to select, ENTER when done):`,
            choices: ticketsInColumn.map(ticket => ({
              name: `${ticket.id} - ${ticket.title}${ticket.priority ? ` [${ticket.priority}]` : ''}`,
              value: ticket.id,
              checked: false,
            })),
      ...
      this.log('')
      if (flags['dry-run']) {
        const scope = columnName ? `for column "${columnName}"` : `for selected tickets`
        this.log(styles.header(`🧪 Dry Run: Spawning agents ${scope}`))
      } else {
        const scope = columnName ? `for column "${columnName}"` : `for selected tickets`
        this.log(styles.header(`🚀 Spawning agents ${scope}`))
      }
      this.log('')
      ...
      // If specific tickets were selected, we need to spawn them individually
      ...
      const result = await spawnForColumn(
        ...
        this.pmoPath,
        {
          ...
          // Pass selected ticket IDs to filter
