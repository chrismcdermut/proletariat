...
export default class Init extends Command {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> --setup manual',
  ];

  static flags = {
    ...machineOutputFlags,
    setup: Flags.string({
      description: 'Setup method for onboarding (claude-code, codex, or manual)',
  ...
  async run(): Promise<void> {
    ...
    if (firstTime || flags.setup) {
      if (jsonMode) {
        // JSON mode: output tool detection and choices for agent-driven onboarding
        ...
        // Interactive mode: run onboarding wizard
        const result = await runOnboardingWizard();

        if (result.method === 'ai') {
          // AI tool handled the setup, we're done
          return;
        }

        // Manual mode: fall through to normal output with guidance
      }
    }
