...
export default class WorkTrello extends PMOCommand {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> --issue trello-abc123',
    '<%= config.bin %> <%= command.id %> --issue trello-abc123 --yes --skip-permissions --display terminal',
  ]

  static flags = {
    ...pmoBaseFlags,
    issue: Flags.string({
      description: 'Trello card key (for example: trello-abc123 or a card ID)',
    }),
    limit: Flags.integer({
      char: 'l',
  ...
  async execute(): Promise<void> {
    ...
    if (!selectedIssue && flags.issue) {
      try {
        // Extract card ID from "trello-xyz" format or use raw ID
        const cardId = flags.issue.replace(/^trello-/i, '')
