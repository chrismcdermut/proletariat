...
export default class WorkTrello extends PMOCommand {
  ...
  static examples = [
    ...
    '<%= config.bin %> <%= command.id %> --issue abc123',
    '<%= config.bin %> <%= command.id %> --issue abc123 --yes --skip-permissions --display terminal',
  ]
  ...
  async execute(): Promise<void> {
    ...
    if (issues.length === 0) {
      return this.handleError('NO_TRELLO_CARDS', 'No open Trello cards found.', { jsonMode, commandName: 'work trello', flags })
    }

    let selectedIssue = issues.find(issue => issue.source.externalId === flags.issue || issue.source.externalKey === flags.issue)
    if (!selectedIssue && flags.issue) {
      try {
        selectedIssue = await getTrelloCardByKey({
          apiKey,
          apiToken,
          boardId: trelloConfig?.boardId,
      ...
      if (!selectedIssue) {
        return this.handleError('TRELLO_CARD_NOT_FOUND', `Trello card "${flags.issue}" was not found.`, { jsonMode, commandName: 'work trello', flags })
      }
    }
    ...
    if (!selectedIssue) {
      const selectedKey = await this.selectFromList({
        ...
        getCommand: issue => buildTrelloCardChoiceCommand(issue.source.externalId, projectId),
