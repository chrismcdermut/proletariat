...
export default class WorkStart extends PMOCommand {
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      // PRLT-1317: Action-level guardrail — validate invocation state
      if (selectedAction && !flags.force) {
        const guardrail = validateActionState({
          validFrom: selectedAction.validFrom,
          db,
        ...
        if (!guardrail.allowed) {
          db.close()
          return handleError('INVALID_STATE', guardrail.message!)
        }
      }
