...
async function verifyTrelloCredentials(
  ...
  const response = await fetchImpl(
    `${TRELLO_API_URL}/1/members/me?key=${encodeURIComponent(apiKey)}&token=${encodeURIComponent(apiToken)}&fields=id,fullName,username`,
    { method: 'GET' },
  )

  if (response.status === 401 || response.status === 403) {
    throw new Error('Invalid or expired Trello API credentials.')
  }
