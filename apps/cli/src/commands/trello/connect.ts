...
): Promise<{ userName: string; username: string }> {
  const response = await fetchImpl(
    `${TRELLO_API_BASE}/1/members/me?key=${encodeURIComponent(apiKey)}&token=${encodeURIComponent(apiToken)}`,
    {
      method: 'GET',
...
async function fetchBoards(
  ...
  const response = await fetchImpl(
    `${TRELLO_API_BASE}/1/members/me/boards?filter=open&fields=name,url&key=${encodeURIComponent(apiKey)}&token=${encodeURIComponent(apiToken)}`,
    {
      method: 'GET',
      headers: { 'Accept': 'application/json' },
    },
...
export default class TrelloConnect extends PMOCommand {
  ...
  static examples = [
    ...
    'TRELLO_API_KEY=... TRELLO_TOKEN=... <%= config.bin %> <%= command.id %>',
  ...
  async execute(): Promise<void> {
    ...
    if (existingConfig && !flags.force) {
      try {
        ...
        if (existingConfig.boardId) {
          this.log(colors.textMuted(`  Board: ${existingConfig.boardId}`))
        }
        this.log('')
    ...
    if (!apiKey || !apiToken) {
      if (jsonMode) {
        outputErrorAsJson(
          'TRELLO_CREDENTIALS_REQUIRED',
          'Trello API key and token required. Set TRELLO_API_KEY and TRELLO_TOKEN environment variables, or run interactively.',
          createMetadata('trello connect', flags),
        )
        this.exit(1)
      }
      ...
      this.log('')
      this.log('Then generate a token using:')
      this.log(colors.textSecondary('  https://trello.com/1/authorize?expiration=never&scope=read,write&response_type=token&key=YOUR_API_KEY'))
      this.log('')
    ...
    try {
      ...
      // Offer board selection in interactive mode
      if (!jsonMode) {
        ...
        if (boards.length > 0) {
          ...
          const { selectedBoard } = await inquirer.prompt([{
            ...
            choices: [
              { name: 'Skip - configure later', value: '' },
              ...boards.map(b => ({ name: b.name, value: b.id })),
            ],
          }])

          if (selectedBoard) {
            saveTrelloBoardId(db, selectedBoard)
            const boardName = boards.find(b => b.id === selectedBoard)?.name
            this.log(colors.textMuted(`  Default board: ${boardName}`))
          }
        }
      }
  ...
  private async handleCheck(
    ...
    try {
      ...
      this.log(colors.textMuted(`  User: ${info.userName} (@${info.username})`))
      if (config.boardId) {
        this.log(colors.textMuted(`  Board: ${config.boardId}`))
      }
    } catch (error) {
