...
export default class TrelloSyncCommand extends PMOCommand {
  ...
  static flags = {
    ...
    list: Flags.string({
      description: 'Trello list ID to create cards in (used with --create-missing)',
    }),
    board: Flags.string({
      description: 'Trello board ID (overrides configured default)',
    }),
  ...
  async execute(): Promise<void> {
    ...
    if (flags.ticket) {
      ...
      if (flags.card) {
        mapper.createOrUpdateMapping(flags.ticket, flags.card, boardId)
      }
      ...
      if (!mapping) {
        ...
        const cardId = await sync.createCardForTicket(ticket!, listId!, boardId)
