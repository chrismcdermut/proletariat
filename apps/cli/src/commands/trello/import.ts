...
import type { TrelloCard } from '../../lib/trello/types.js'
import {
...
export default class TrelloImport extends PMOCommand {
  ...
  async execute(): Promise<void> {
    ...
    this.log(colors.textMuted(`Fetching cards from Trello board "${config.boardName || config.boardId}"...`))

    let trelloCards: TrelloCard[]
    try {
      trelloCards = await client.getBoardCards(config.boardId, { limit: flags.limit })
    } catch (error: unknown) {
    ...
    const alreadyImported: TrelloCard[] = []
