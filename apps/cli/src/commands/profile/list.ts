...
export default class ProfileList extends PMOCommand {
  ...
  async execute(): Promise<void> {
    ...
    if (profiles.length === 0) {
      ...
      this.log(styles.muted('Create one with: prlt profile create "my-profile"'));
      return;
    }
  ...
  private printProfile(profile: AgentProfile): void {
    ...
    if (profile.startHook) {
      details.push('start hook');
    }
    if (profile.endHook) {
      details.push('end hook');
    }
    if (profile.systemPrompt) {
      details.push('system prompt');
    }
    if (profile.repos?.length) {
      details.push(`repos: ${profile.repos.join(', ')}`);
    }
    if (profile.mcpServers?.length) {
      details.push(`MCP servers: ${profile.mcpServers.length}`);
    }
    if (details.length > 0) {
