...
export default class TicketList extends Command {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      // When provider is not PMO (e.g., Linear), always use cross-project view
      const isExternalProvider = provider.name !== 'pmo';
      if (flags.all || isExternalProvider) {
        // Cross-project view (also used for external providers like Linear)
        switch (flags.format) {
          case 'json':
