...
describe('Trello config', () => {
  ...
  describe('saveTrelloBoardId', () => {
    it('saves board id and name', () => {
      saveTrelloConfig(db, { apiKey: 'k', apiToken: 't' })
      saveTrelloBoardId(db, 'b123', 'Test Board')

      const config = loadTrelloConfig(db)
      expect(config?.boardId).to.equal('b123')
      expect(config?.boardName).to.equal('Test Board')
    })
  })
