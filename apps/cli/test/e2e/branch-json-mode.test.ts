...
describe('Branch Commands JSON Mode', () => {
  ...
  describe('branch (main menu) --machine', () => {
    it('should output valid JSON with prompt schema', async () => {
      const output = await execInProcess('branch --machine');
    ...
    it('should work with -m shorthand', async () => {
      const output = await execInProcess('branch -m');
  ...
  describe('branch list --format json', () => {
    ...
    it('should work with -f json shorthand', async () => {
      const output = await execInProcess('branch list -f json');
  ...
  describe('branch create --machine', () => {
    it('should output prompt JSON for mode selection', async () => {
      const output = await execInProcess('branch create --machine');
      const json = extractJson<{
        prompt: { type: string; name: string; message: string; choices: Array<{ name: string; value: string }> };
        metadata: { command: string; flags: Record<string, unknown> };
      }>(output);
    ...
    it('should work with -m shorthand', async () => {
      const output = await execInProcess('branch create -m');
      const json = extractJson<{ prompt: { type: string }; metadata: { flags: { machine: boolean } } }>(output);
    ...
    it('should create branch when type and description flags provided', async () => {
      const output = await execInProcess('branch create -t feat -d test-feature --force');
  ...
  describe('branch validate --machine', () => {
    it('should validate current branch', async () => {
      const output = await execInProcess('branch validate');
    ...
    it('should validate provided branch name', async () => {
      const output = await execInProcess('branch validate feat/chris/add-auth');
    ...
    it('should report invalid branch format', async () => {
      const output = await execInProcess('branch validate invalid-branch-name');
  ...
  describe('End-to-end agent flows (--machine flag)', () => {
    ...
    describe('branch create - full agent flow', () => {
      ...
      it('should complete flow with all flags provided directly (custom)', async () => {
        const result = await execInProcess('branch create -t feat -d agent-test-branch --force');
      ...
      it('should complete flow with ticket flag provided directly', async () => {
        // Use the test ticket
        const result = await execInProcess('branch create -T TKT-001 --force');
...
describe('Branch Commands - JSON Mode Compatibility', () => {
  ...
  it('--json and --machine should be interchangeable', async () => {
    const jsonOutput = await execInProcess('branch --json');
    const machineOutput = await execInProcess('branch --machine');
  ...
  it('branch create --json should match --machine', async () => {
    const jsonOutput = await execInProcess('branch create --json');
    const machineOutput = await execInProcess('branch create --machine');
