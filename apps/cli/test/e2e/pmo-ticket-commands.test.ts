...
describe('@smoke PMO Ticket Commands E2E Tests', () => {
  ...
  describe('prlt ticket create', () => {
    ...
    it('should default column to Backlog in JSON mode when not provided', async () => {
  ...
  describe('prlt ticket link', () => {
    ...
    it('should link ticket to epic via DB', async () => {
    ...
    it('should unlink ticket from epic via DB', async () => {
    ...
    it('should show ticket link subcommands', async () => {
      const output = await execInProcess('ticket link --help');
...
/**
 * Tests for prlt ticket update command (enhanced with title, description, labels, status)
 * Uses production schema to avoid schema drift issues.
 */
describe('@smoke prlt ticket update', () => {
  ...
  it('should update ticket description from file', async () => {
    ...
    const descFile = path.join(env.testDir, 'description.md');
  ...
  it('should update ticket status', async () => {
    ...
    const updatedTicket = db.prepare(`
      ...
      JOIN pmo_workflow_statuses s ON s.id = t.status_id
      WHERE t.id = ?
    `).get(ticket.id) as { status_name: string };
    expect(updatedTicket.status_name).to.equal('In Progress');
  });
  ...
  it('should update multiple fields at once', async () => {
    ...
    const labelsRow = db.prepare('SELECT labels FROM pmo_tickets WHERE id = ?').get(ticket.id) as { labels: string };
    const labels = JSON.parse(labelsRow.labels) as string[];
    expect(labels.sort()).to.deep.equal(['api', 'backend']);
  });
  ...
  it('should output JSON in --json mode', async () => {
    ...
    const output = await execInProcess(`ticket update ${ticket.id} --title "Updated JSON" --json`);

    const json = JSON.parse(output);
    expect(json.success).to.equal(true);
    expect(json.ticket).to.be.an('object');
    expect(json.ticket.id).to.equal(ticket.id);
  });
  ...
  it('should error when description and description-file are both provided', async () => {
    ...
    let output: string;
    try {
      output = await execInProcess(`ticket update ${ticket.id} --description "inline" --description-file /tmp/nonexistent --json`);
    } catch (error) {
      output = error instanceof Error ? error.message : String(error);
    }
    expect(output).to.contain('Cannot use both --description and --description-file');
  });
});
