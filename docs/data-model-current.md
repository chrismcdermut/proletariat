# PRLT Data Model (Current)

Complete data model for the PRLT PMO and Agent/Execution systems as currently implemented.

## Overview

PRLT has two main subsystems:
1. **PMO System** - Project management (tickets, epics, specs, statuses)
...
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│                              PRLT CURRENT DATA MODEL                                 │
│                          (PMO + Agent/Execution Systems)                             │
└─────────────────────────────────────────────────────────────────────────────────────┘
...
Workspace
    ...
    └──▶ PMO (if has_pmo = true)
             ...
             ├──▶ Projects (1:N)
             │        │
             │        ├──▶ Statuses (1:N) ◀── per-project copies
             │        │
             │        ├──▶ Epics (1:N)
             │        │        └──▶ Tickets (1:N)
             │        │
...
Currently, each project has its own copy of statuses:
...
This means:
- 10 projects with identical workflows = 50 status rows (5 per project)
- Changing a workflow requires updating each project individually
