...
## Core Concepts
- **Assignee**: Person or agent assigned to do the work
- **Claiming**: Taking a ticket and moving it to In Progress
- **Agent Workflow**: Agents are assigned tickets by orchestrators

## Namespace
All work commands are under the `prlt ticket` namespace but handle workflow/orchestration:
- `prlt ticket assign` - Assign ticket to user/agent
- `prlt ticket claim` - Claim ticket and start work
- `prlt ticket own` - Take responsibility (future)
- `prlt ticket execute` - Spin up agent to work (future)

## Command Overview

| Command                           | Purpose                                |
| --------------------------------- | -------------------------------------- |
| `prlt ticket assign [id] [agent]` | Assign ticket to user/agent            |
| `prlt ticket claim [id]`          | Claim ticket (move to In Progress)     |
| `prlt ticket own [id]`            | Take ownership (responsibility)        |
| `prlt ticket execute [id]`        | Execute ticket (spin up agent/start)   |
