...
- [ ] **TKT-073** [[TKT-073]] Fix branch create command for non-interactive use
      **Category:** fix
      ***
      The prlt branch create command fails when the branch name doesn't follow conventional format because it prompts interactively for confirmation. This blocks non-interactive usage (e.g., from AI agents or scripts).
      
      Current behavior: Command prompts 'Branch name doesn't follow conventional format. Continue anyway?' with interactive menu.
      
      Expected behavior:
      - Add a --yes flag to skip the confirmation prompt
      - Or: accept the branch name silently when --force is already provided
      - The command should work in non-interactive environments without hanging
      
      Workaround: Use manual git commands instead.
